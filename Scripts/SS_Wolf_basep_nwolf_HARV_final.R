################################################################################
  ...
  #  Covariates on occupancy: area-weighted number of wolves harvested (HARV)
...
################################################################################
  ...
  sink("SS_Wolf_basep_nwolf_HARV_final.txt")
  cat("
  model {
  ...
  #  logit() transforms the logit scale to the probability scale
    ...
    for(i in 1:nSite){
       for (k in 1:K){
        logit.psi[i,k] <- B0.psi[k] + B1.HARV*HARV[i,k]
  ...
  #  Annual pack (n.packs) and wolf (n.wolves) abundance, area occupied (occ.area),
  #  and mean # packs across years (mean.pack)
...
################################################################################
  ...
  params <- c("B0.psi", "B1.HARV", "B0.p.rend", "B0.p.hunt", "n.occ", "k.psi", "k.p.rnd",
              "k.p.hs", "n.packs", "mean.pack", "n.wolves", "psi", "ann.occ", "mean.occ") #, "loglik"
  ...
  # Call JAGS 
  out_HARV <- jags(win.data, inits, params, "SS_Wolf_basep_nwolf_HARV_final.txt", n.chains=nc, 
              n.thin=nt, n.iter=ni, n.burnin=nb, jags.module = c("glm","dic"))
  
  jag.sum <- out_HARV$BUGSoutput$summary
  #write.table(x=jag.sum, file="C:/Sarah B/Thesis POM/Final_Models_4_Thesis/Output/SS_Wolf_basep_nwolf_HARV_final_updated.txt", sep="\t")
  
  print(out_HARV, dig=2)
  
  # #  Pull MCMC chains together to compute WAIC
  # mcmc_out_HARV <- as.mcmc(out_HARV)
  # mcmc_all_HARV <- rbind(mcmc_out_HARV[[1]], mcmc_out_HARV[[2]], mcmc_out_HARV[[3]])
  # dim(mcmc_all_HARV)
  # 
  # #  Computes WAIC value for model comparison
  # library(loo)
  # 
  # SS_Wolf_basep_nwolf_HARV_WAIC <- waic(mcmc_all_HARV)

  #print(SS_Wolf_basep_nwolf_HARV_final)
  SS_Wolf_basep_nwolf_HARV_final <- out_HARV
  #save(SS_Wolf_basep_nwolf_HARV_final, file ="C:/Sarah B/Thesis POM/Final_Models_4_Thesis/Output/SS_Wolf_basep_nwolf_HARV_final_updated.RData")
