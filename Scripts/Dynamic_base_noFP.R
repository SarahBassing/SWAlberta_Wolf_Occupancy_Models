...
sink("Dynamic_Wolf_base_noFP.txt")
cat("
model {
...
#  Need to transform to probability scale later
  ...
  # Prior for detection probability (only varies by year)

  for(k in 1:K){
    B0.p[k] ~ dnorm(0,0.001)
  }#k
...
#     for (j in 1:nOcc){
#       for(k in 1:K){	
#         logit.p[i,j,k] <- B0.p[k]+B7.p.SVY*SVY[i,j,k]     
...
# inits <- function(){list(z=zst, B0.psi1=0, B0.phi.const=0, B0.gamma.const=-2,
#                          B0.p=c(-0.7,-0.7,-0.7))}
inits <- function(){list(z=zst, B0.psi1=0, B0.phi=c(0,0), B0.gamma=c(-2,-2),
                         B0.p=c(-0.7,-0.7,-0.7))}

# Parameters to keep track of and report
#params <- c("psik", "gammak", "phik", "growthrk", "turnoverk", "n.occ", "k.p.rnd",
#            "k.p.hs", "B0.psi1", "B0.phi", "B0.gamma", "B0.p", "B7.p.SVY")
params <- c("psik", "gammak", "phik", "growthrk", "turnoverk", "n.occ", "k.p.rnd",
            "k.p.hs", "B0.psi1", "B0.phi", "B0.gamma", "B0.p")
...
# Call JAGS 
out <- jags(win.data, inits, params, "Dynamic_Wolf_base_noFP.txt", n.chains=nc, 
            n.thin=nt, n.iter=ni, n.burnin=nb, jags.module = c("glm","dic"))

jag.sum <- out$BUGSoutput$summary
#write.table(x=jag.sum, file="C:/Sarah B/Thesis POM/Model_Outputs/Dynamic_Wolf_base_noFP.txt", sep="\t")
